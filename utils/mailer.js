...
const mailer = (res, next, user, token, method) => {
  ...
  const optionsVerify = {
    ...
    <p> ${user.nickname，您好: <br />
       ...
       驗證成功後，即可登入您的帳戶 <br />
        <a href="${process.env.FRONTEND_URL}/auth/verify-email?nickname${user.nnicknametoken=${token}&email=${user.email}">驗證連結</a><br />
       驗證連結於一個小時後逾期<br />
  ...
  const optionsForgot = {
    ...
    <p> ${user.nickname，您好: <br />
       ...
       點擊連結後，即可重新設定密碼<br />
        <a href="${process.env.FRONTEND_URL}/auth/reset-password?nickname${user.nnicknametoken=${token}&email=${user.email}">重設密碼連結</a><br />
       驗證連結於一個小時後逾期<br />
