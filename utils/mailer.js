...
const mailer = (res, next, user, token, method) => {
  ...
  const optionsVerify = {
    ...
    <p> ${user.nickName}，您好: <br />
       ...
       驗證成功後，即可登入您的帳戶 <br />
        <a href="${process.env.BACKEND_URL}/auth/verify-email?nickName=${user.nickName}&token=${token}">驗證連結</a><br />
       驗證連結於一個小時後逾期<br />
  ...
  const optionsForgot = {
    ...
    <p> ${user.nickName}，您好: <br />
       ...
       點擊連結後，即可重新設定密碼<br />
        <a href="${process.env.BACKEND_URL}/reset-password?nickName=${user.nickName}&token=${token}">重設密碼連結</a><br />
       驗證連結於一個小時後逾期<br />
