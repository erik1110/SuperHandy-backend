...
const mailer = (res, next, user, token, method) => {
    ...
    const optionsVerify = {
        ...
        html: `
    <h2>驗證用戶信箱</h2>
    <p> ${user.nickename}，您好: <br />
       感謝您註冊 SuperHandy！為了確保您的帳戶安全，請使用以下連結並完成驗證流程：<br />
       驗證成功後，即可登入您的帳戶 <br />
        <a href="${process.env.FRONTEND_URL}/auth/verify-email?nickename=${user.nickename}&token=${token}&email=${user.email}">驗證連結</a><br />
       驗證連結於一個小時後逾期<br />
    ...
    <p> ${user.nickename}，您好: <br />
       ...
       點擊連結後，即可重新設定密碼<br />
        <a href="${process.env.FRONTEND_URL}/auth/reset-password?nickename=${user.nickename}&token=${token}&email=${user.email}">重設密碼連結</a><br />
       驗證連結於一個小時後逾期<br />
