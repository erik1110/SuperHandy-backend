...
const tasks = {
    ...
    createDraft: handleErrorAsync(async (req, res, next) => {
        ...
        console.log('check point taskValidator', taskValidator);
        ...
        if (!draftModel) {
            return res.status(404).json({ message: 'check fail, 應該不會走到這裡(tasks.createDraft)' });
        }
        res.status(200).json(
            getHttpResponse({
                data: draftModel,
    ...
    getDraft: handleErrorAsync(async (req, res, next) => {
        ...
        if (taskModel.status !== 'draft') return next(appError(400, '40103', '此任務之狀態不可編輯'));
        //return task
